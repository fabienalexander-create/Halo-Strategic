...
Flat repository root: every HTML page, every static asset (icons, manifest, one portrait image), and a `docs/` directory holding all documentation. No `src/`, `assets/`, or `build/` separation exists. Pages were documented as generated through a shared Python template system (`common.py` plus one build script per page); as of 2026-07-26 that tooling could not be located anywhere in this repository's history or in related local locations searched (see docs/ARCHITECTURAL_DECISIONS.md, ADR-001 Amendment). Until it is located or rebuilt, treat the 12 HTML files as directly maintained, with shared elements (header, footer, `<style>` block) kept consistent by hand across pages rather than by a generator.
