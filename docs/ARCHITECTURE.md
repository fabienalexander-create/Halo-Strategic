...
- **The build system does not scale to one-file-per-page indefinitely, and this risk is now live, not hypothetical.** Article #1 was published 2026-07-26 by hand-copying the shared boilerplate into a new file, the same direct-maintenance approach used everywhere else on the site (there was never a Python build script to begin with, see ADR-001 Amendment; this section's original wording assumed one existed). That approach is fine for one article. It was already flagged here as not scaling to 100+ articles, and publishing article #1 this way does not resolve that, it just means the threshold is one article closer. Before publishing article #2 or #3, revisit whether a lightweight templating approach (even a simple script that generates the shared boilerplate rather than hand-copying it) is worth building, rather than accumulating hand-copied files until it becomes painful.
