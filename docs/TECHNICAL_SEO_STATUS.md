...
Per docs/ROADMAP.md, remediation of these findings is split across Sprint 1 (nav/footer/GTM parity on the two lagging pages, Organization and FAQPage schema once approved, and closing the two unaudited pages) and Sprint 2 (sitemap.xml, robots.txt, favicon/manifest consistency, Core Web Vitals baseline, all pending the URL strategy decision). Findings 7, 8, 13, and now 6 (Organization and FAQPage portions) were closed on 2026-07-26 (see below), completing Sprint 1 in full. LocalBusiness schema remains deferred pending a registered business address (docs/ARCHITECTURAL_DECISIONS.md, ADR-007); the remaining findings are Sprint 2 scope.
...
| 6 | Structured data (schema.org) missing entirely | Organization schema implemented site-wide (12 of 12 pages) and FAQPage implemented on commercial-audit.html, both 2026-07-26 per ADR-007. LocalBusiness deferred pending a registered address; Article deferred pending Insights, both unchanged from docs/ARCHITECTURE.md's original recommendation | Yes, all 12 Organization blocks and the FAQPage block validated as syntactically correct JSON-LD; FAQPage entries confirmed to match the visible `<details>/<summary>` FAQ content verbatim | None detected |
