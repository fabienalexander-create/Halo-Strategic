...
**Tracker: `halo-growth-hvac/hvac-qualified-lead-tracker.xlsx`**, built 2026-09-15, Dashboard/Campaign Summary/Leads/Make.com Setup tabs, formulas verified via a real recalculation pass. Leads should land in the "Leads" tab automatically once connected via **two Make.com scenarios**, one per ad/form, each writing a static Concept value ("A" or "C") into the Google Sheets row, **then sending Fabien an email notification** with the lead's details (Fabien's request, added 2026-09-13, since the 15-minute response SLA depends on him actually knowing a lead arrived, not just it sitting in a sheet). Exact steps on that file's "Make.com Setup" tab, not yet built or tested in this session, no LinkedIn, Make.com, or Gmail access here. Fallback meanwhile: paste leads in manually from a Campaign Manager CSV export. This needs the file imported into Google Sheets first, Make writes to a live Google Sheet, not a local .xlsx.
